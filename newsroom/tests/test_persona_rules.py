...
def test_should_build_a_byline_that_discloses_ai_authorship(
    ...
    assert personas.byline_for("kolka") == "Kolka · AI correspondent, Maritime & Trade"
...
def test_should_not_accept_a_byline_missing_the_disclosure(
    ...
    assert personas.byline_discloses_ai("Kolka · Maritime correspondent") is False
...
def test_should_route_a_section_to_its_correspondent(personas: PersonaRegistry) -> None:
    ...
    assert personas.byline_for_section("environment").startswith("Ristna")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Kolka travelled to the port of Klaipėda.",
...
def test_should_report_a_byline_missing_the_disclosure_as_a_problem(
    ...
    problems = personas.validate_output(
        "kolka", "The data shows cargo fell.", byline="Kolka · Maritime correspondent"
    )
