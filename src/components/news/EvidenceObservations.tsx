...
export function EvidenceObservations({ data }: { data: EvidenceNormalized }) {
  ...
  const shown = allPeriods ? rows : rows.slice(0, 12);
  ...
  return (
    <section className="evidence-section" aria-labelledby="observations-heading">
      <div className="evidence-section-heading">
        ...
        <label className="evidence-control text-ui">Country
      ...
      <p className="text-ui news-muted" role="status">Showing {shown.length} of {rows.length} observation months for {EVIDENCE_COUNTRIES[country]}. The CSV includes all countries and periods.</p>
      ...
      {rows.length > 12 && <button type="button" className="site-action text-ui" aria-expanded={allPeriods} onClick={() => setAllPeriods(value => !value)}>
