...
describe('the part/whole graph, before anything is asserted with it', () => {
  it('read the containment table rather than keeping a second copy of it', () => {
    ...
    // Arrivals and nights now use separate cubes, leaving six declared families.
    expect(families, 'every current containment family must be parsed').toHaveLength(6);
